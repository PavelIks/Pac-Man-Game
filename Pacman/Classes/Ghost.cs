...
namespace Pacman
{
    public class Ghost
    {
        ...
        private void MoveGhosts(int x)
        {
            ...
            else
            {
                ...
                while (!CanMove)
                {
                    ...
                    if (!CanMove)
                    {
                        Change_Direction(Direction[x], x);
                    }
                ...
                if (CanMove)
                {
                    switch (Direction[x])
                    {
                        case 1: GhostImage[x].Top -= 16; yCoordinate[x]--;
                            {
                                break;
                            }
                        case 2: GhostImage[x].Left += 16; xCoordinate[x]++;
                            {
                                break;
                            }
                        case 3: GhostImage[x].Top += 16; yCoordinate[x]++;
                            {
                                break;
                            }
                        case 4: GhostImage[x].Left -= 16; xCoordinate[x]--;
                            {
                                break;
                            }
                    }
                    switch (State[x])
                    {
                        case 0: GhostImage[x].Image = GhostImages.Images[x * 4 + (Direction[x] - 1)];
                            {
                                break;
                            }
                        case 1:
                            ...
                            break;
                        case 2: GhostImage[x].Image = GhostImages.Images[18];
                            {
                                break;
                            }
                    }
        ...
        private bool direction_ok(int x, int y, int ghost)
        {
            ...
            if (x < 0)
            {
                xCoordinate[ghost] = 27; GhostImage[ghost].Left = 429; return true;
            }
            if (x > 27)
            {
                xCoordinate[ghost] = 0;
        ...
        private void Other_Direction(int direction, int ghost)
        {
            ...
            if (Form1.gameboard.Matrix[yCoordinate[ghost], xCoordinate[ghost]] < 4)
            {
                ...
                switch (direction)
                {
                    case 1: case 3: directions[2] = direction_ok(x + 1, y, ghost); directions[4] = direction_ok(x - 1, y, ghost);
                        {
                            break;
                        }
                    case 2: case 4: directions[1] = direction_ok(x, y - 1, ghost); directions[3] = direction_ok(x, y + 1, ghost);
                        {
                            break;
                        }
                }
